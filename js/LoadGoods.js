const loadGoods = (path) => {
    axios.get(path)
        .then(resp => {
            const { data, status } = resp;
            console.log("Resp status", status);
            const list_news = document.getElementById("Goods");
            data.forEach(item => {
                    list_news.innerHTML += 
                    `
                <div class="max-w-sm scale-85  bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                    <a href="#">
